...
namespace Prism.Parsing.Code.Signatures
{
	...
	public class FullTypeInfo
	{
		...
		public TypeToken ToTypeToken()
		{
			...
			if (InnerType.IsConst) properties |= TypeProperties.Const;
			if (PointerCount != 0) properties |= TypeProperties.Pointer;
			if (PointerCount != 0 && PointerData[PointerCount - 1].IsConst) properties |= TypeProperties.ConstPointer;
			if (IsReference) properties |= TypeProperties.Reference;
			if (IsStatic) properties |= TypeProperties.Static;
			if (IsInlined) properties |= TypeProperties.Inline;
			if (IsMutable) properties |= TypeProperties.Mutable;
			if (IsVolatile) properties |= TypeProperties.Volatile;

			return new TypeToken(InnerType.TypeName, properties);
		}
	}
	...
	public class TypeSignature
	{
		...
		public static bool TryGetFullTypeInfo(string typeString, out FullTypeInfo typeInfo)
		{
			...
			if (typeString.Contains("mutable"))
			{
				typeString = typeString.Replace("mutable", "").Trim();
				typeInfo.IsMutable = true;
			}
