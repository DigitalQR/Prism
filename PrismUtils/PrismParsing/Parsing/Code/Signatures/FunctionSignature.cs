...
namespace Prism.Parsing.Code.Signatures
{
	public class FunctionInfo
	{
		...
		public FunctionProperties ToFunctionProperties()
		{
			FunctionProperties properties = FunctionProperties.None;

			if (IsConst) properties |= FunctionProperties.Const;
			if (IsVirtual) properties |= FunctionProperties.Virtual;
			if (IsStatic) properties |= FunctionProperties.Static;
			if (IsInlined) properties |= FunctionProperties.Inline;
			if (IsAbstract) properties |= FunctionProperties.Abstract;
			if (IsExplicit) properties |= FunctionProperties.Explicit;
		...
		public static bool TryParse(ref int braceBlockDepth, long firstLine, string content, SafeLineReader reader, out SignatureInfo sigInfo)
		{
			...
			if (!string.IsNullOrEmpty(searchString) && searchString.Length >= 4)
			{
				...
				if (!searchString.StartsWith(",") && (searchString.StartsWith("::") ? true : !searchString.StartsWith(":")) && searchString.EndsWith(")"))
				{
					...
					if (searchString.Contains("explicit"))
					{
						data.IsExplicit = true;
						searchString = searchString.Replace("explicit", "").Trim();
					}
